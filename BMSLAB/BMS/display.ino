void showMeasurementValues()
{
  ...
  for (i = 1; i <= 4; i++)
  {
    ...
    writeText(i*68-52, 22, 1, String(getCellVoltage(i),3)+"V", colCell(i)); //schriftgröße position und einheit
    writeText(i*68-52, 40, 1, strLen(String(getCellTemp(i),1),4)+"C", colTemp(i)); //schriftgröße position und einheit
  }
  // write pack current on screen
  writeText(280,  4, 1, "I/A",ILI9341_LIGHTGREY); //neuer Name
  writeText(280, 22, 1, strLen(String(getPackCurrent(),1),4),ILI9341_BLUE); //neue Schriftgröße Position
}
