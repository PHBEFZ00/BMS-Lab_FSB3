void showMeasurementValues()
{
  ...
  for (i = 1; i <= 4; i++)
  {
    ...
    writeText(i*68-66, 22, 1, strLen(String(getCellVoltage(i),3),6), colCell(i));     // Nachkommastellen bearbeitet auf 3
    writeText(i*68-66, 40, 1, strLen(String(getCellTemp(i),2),5), colTemp(i));        // Nachkommastellen bearbeitet auf 2
  }
  // write pack current on screen
  writeText(280,  4, 2, "I/A",ILI9341_LIGHTGREY);
  writeText(280, 22, 1, strLen(String(getPackCurrent(),1),5),ILI9341_BLUE);           // Nachkommastellen bearbeitet auf 1
}
